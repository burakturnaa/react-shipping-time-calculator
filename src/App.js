...
import {Button, Grid, Card, Modal, Typography, Box, TextField, Tooltip,OutlinedInput ,MenuItem,FormControl,Select,InputAdornment,Stack  } from '@mui/material';
...
function App() {
  ...
  const handleClose = () => setOpen(false);
    ...
    const timestampToDate = (timestamp, separator) =>{
      let new_date_day = new Intl.DateTimeFormat('en-US', {day: '2-digit'}).format(timestamp);
      let new_date_month = new Intl.DateTimeFormat('en-US', { month: 'long'}).format(timestamp);
      let new_date_year = new Intl.DateTimeFormat('en-US', {year: 'numeric'}).format(timestamp);
      return new Date( new_date_day + separator + new_date_month + separator + new_date_year);
    }
    ...
    const calculateTime = (event) => {
      ...
      if (selected_ship_date == "" || selected_fabric_type == "" || selected_quantity == "" || selected_quantity == 0) {
        ...
        let new_date = timestampToDate(calculated_date,"/");
        ...
        new_date =  timestampToDate(new_date," ");
        ...
        setCalculatedTime(dateFormat(new_date, "dd mmmm yyyy"))
      }
    }
